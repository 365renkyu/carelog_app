...
class _RecordEditScreenState extends ConsumerState<RecordEditScreen> {
                ...
                Row(
                  ...
                  children: [
                    ...
                    TextButton.icon(
                      ...
                      label: const Text('追加'),
                      onPressed: () => _showSleepLogDialog(context, notifier, state),
                    ),
                  ],
                ),
                ...state.sleepLogs.map(
                  (log) => SleepTimeRangeTile(
                    ...
                    onEdit: () => _showSleepLogDialog(
                        context, notifier, state, log: log),
                ...
                Row(
                  ...
                  children: [
                    ...
                    TextButton.icon(
                      icon: const Icon(Icons.add, size: 18),
                      label: const Text('追加'),
                      onPressed: () => _showMealLogDialog(context, notifier, state),
                    ),
                  ],
                ),
                ...state.mealLogs.map(
  ...
  Future<void> _save(RecordEditNotifier notifier) async {
    ...
    notifier.setAchievements(_achievementsController.text);
    notifier.setCuteMoments(_cuteMomentsController.text);
    notifier.setConcerns(_concernsController.text);
    notifier.setTherapyMemo(_therapyMemoController.text);
